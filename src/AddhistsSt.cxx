//clang++ -Isrc -std=c++17 -march=native -pipe -O3 -Wall -Wextra -Wpedantic -o build/addhistsSt src/AddhistsSt.cxx `root-config --libs` -lm
...
void addhistsSt(const channel ch){
	// do tzq first so that tpr & tln are not null
	if(munu == ch) {NPLc = "munu";NPLds = "_st";}
...
*/
	...
	/*NPLds = "cms"; // now do the odd one out
		...
		dc.Close();*/
	for(dataSource ds:{st,stb}){// to go through all ds get the files open
		if(st == ds)continue;
...
*/
	...
	ftpt_w->SetName(( "ttbSF_"   + NPLc +"__ST").c_str());
	fcmtet->SetName(("cmet_sEt_" + NPLc +"__ST").c_str());
	fcmtpt->SetName(("cmet__pt_" + NPLc +"__ST").c_str());
	fcmtph->SetName(("cmet_phi_" + NPLc +"__ST").c_str());
	fmteta->SetName(( "met_sEt_" + NPLc +"__ST").c_str());
	fmt_pt->SetName(( "met__pt_" + NPLc +"__ST").c_str());
	ft__pt->SetName(( "ttop_pt_" + NPLc +"__ST").c_str());
	ft_mas->SetName((     "tTm_" + NPLc +"__ST").c_str());
	fWinvm->SetName(("W_invariant_mass_" + NPLc +"__ST").c_str());
        ftWinm->SetName((     "tWm_" + NPLc +"__ST").c_str());
	fev_sf->SetName((     "ev_w_"+ NPLc +"__ST").c_str());
	fz_mas->SetName((    "zmas_" + NPLc +"__ST").c_str());
	fzwdph->SetName((  "Z_W_Delta_Phi_" + NPLc +"__ST").c_str());
...
//	f__npl->SetName(("npl_" + NPLc +"__ST").c_str());
        fnbjet->SetName(("nbjet_" + NPLc +"__ST").c_str());
	...
	fjtmas->SetName((NPLc +"__ST_" + "fin_jets_mas").c_str());
	flp_pt->SetName((NPLc +"__ST_" + "lep__pt").c_str());
	flpeta->SetName((NPLc +"__ST_" + "lep_eta").c_str());
	flpphi->SetName((NPLc +"__ST_" + "lep_phi").c_str());
	flpmas->SetName((NPLc +"__ST_" + "lep_mas").c_str());
	fb__pt->SetName((NPLc +"__ST_" + "bjet__pt").c_str());
	fb_eta->SetName((NPLc +"__ST_" + "bjet_eta").c_str());
	fb_phi->SetName((NPLc +"__ST_" + "bjet_phi").c_str());
	fb_mas->SetName((NPLc +"__ST_" + "bjet_mas").c_str());

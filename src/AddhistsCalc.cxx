...
void AddhistsCalc(const channel ch){
	...
	for(dataSource ds:dataSourceAll){// to go through all ds get the files open
		if(tzq == ds || met == ds || zjt1 == ds || cms == ds)continue;
		if(munu == ch && wjqq == ds)continue;
...
*/
	...
	TFile hf(("histo/BDT_NPL_run_" + NPLc + "_.root").c_str(),"RECREATE");
	if(0<debug) std::cout<<"file created"<<std::endl;
	fbtagw->SetName(("btag_w_"   + NPLc +"_NPL").c_str());
	flp_sf->SetName(("mostSF_"   + NPLc +"_NPL").c_str());
	ftpt_w->SetName(( "ttbSF_"   + NPLc +"_NPL").c_str());
	fcmtet->SetName(("cmet_sEt_" + NPLc +"_NPL").c_str());
	fcmtpt->SetName(("cmet__pt_" + NPLc +"_NPL").c_str());
	fcmtph->SetName(("cmet_phi_" + NPLc +"_NPL").c_str());
	fmteta->SetName(( "met_sEt_" + NPLc +"_NPL").c_str());
	fmt_pt->SetName(( "met__pt_" + NPLc +"_NPL").c_str());
	...
	fjtmas->SetName((NPLc +"_NPL_" + "fin_jets_mas").c_str());
	flp_pt->SetName((NPLc +"_NPL_" + "lep__pt").c_str());
	flpeta->SetName((NPLc +"_NPL_" + "lep_eta").c_str());
	flpphi->SetName((NPLc +"_NPL_" + "lep_phi").c_str());
	flpmas->SetName((NPLc +"_NPL_" + "lep_mas").c_str());
	fb__pt->SetName((NPLc +"_NPL_" + "bjet__pt").c_str());
	fb_eta->SetName((NPLc +"_NPL_" + "bjet_eta").c_str());
	fb_phi->SetName((NPLc +"_NPL_" + "bjet_phi").c_str());
	fb_mas->SetName((NPLc +"_NPL_" + "bjet_mas").c_str());

//clang++ -Isrc -std=c++17 -march=native -pipe -O3 -Wall -Wextra -Wpedantic -o build/addhiststtb src/Addhiststtb.cxx `root-config --libs` -lm
...
void addhiststtb(const channel ch){
	...
	if(munu == ch) {NPLc = "munu";NPLds = "ttb";}
...
*/
	...
	/*NPLds = "cms"; // now do the odd one out
		...
		dc.Close();*/
	for(dataSource ds:{ttb,ttl,ttj}){// to go through all ds get the files open
		if(ttb == ds)continue;
...
*/
	...
	TFile hf(("histo/BDT_" + NPLc + "_ttbar.root").c_str(),"RECREATE");
	if(0<debug) std::cout<<"file created"<<std::endl;
	fbtagw->SetName(("btag_w_"   + NPLc +"_ttb").c_str());
	flp_sf->SetName(("mostSF_"   + NPLc +"_ttb").c_str());
	ftpt_w->SetName(( "ttbSF_"   + NPLc +"_ttb").c_str());
	fcmtet->SetName(("cmet_sEt_" + NPLc +"_ttb").c_str());
	fcmtpt->SetName(("cmet__pt_" + NPLc +"_ttb").c_str());
	fcmtph->SetName(("cmet_phi_" + NPLc +"_ttb").c_str());
	fmteta->SetName(( "met_sEt_" + NPLc +"_ttb").c_str());
	fmt_pt->SetName(( "met__pt_" + NPLc +"_ttb").c_str());
	ft__pt->SetName(( "ttop_pt_" + NPLc +"_ttb").c_str());
	ft_mas->SetName((     "tTm_" + NPLc +"_ttb").c_str());
	fWinvm->SetName(("W_invariant_mass_" + NPLc +"_ttb").c_str());
        ftWinm->SetName((     "tWm_" + NPLc +"_ttb").c_str());
	fev_sf->SetName((     "ev_w_"+ NPLc +"_ttb").c_str());
	fz_mas->SetName((    "zmas_" + NPLc +"_ttb").c_str());
	fzwdph->SetName((  "Z_W_Delta_Phi_" + NPLc +"_ttb").c_str());
	fzmdph->SetName(("Z_MET_Delta_Phi_" + NPLc +"_ttb").c_str());
...
//	f__npl->SetName(("npl_" + NPLc +"_ttb").c_str());
	...
	fjtmas->SetName((NPLc +"_ttb_" + "fin_jets_mas").c_str());
	flp_pt->SetName((NPLc +"_ttb_" + "lep__pt").c_str());
	flpeta->SetName((NPLc +"_ttb_" + "lep_eta").c_str());
	flpphi->SetName((NPLc +"_ttb_" + "lep_phi").c_str());
	flpmas->SetName((NPLc +"_ttb_" + "lep_mas").c_str());
	fb__pt->SetName((NPLc +"_ttb_" + "bjet__pt").c_str());
